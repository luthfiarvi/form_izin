        ...
        <div class="max-w-3xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900">
                    ...
                    <form method="post" action="{{ route('admin.users.update', ['user' => $user], false) }}" enctype="multipart/form-data">
                        @csrf
                        @method('put')

                        <div class="mb-4">
                            <label class="block text-sm mb-1">Nama</label>
                            <input type="text" name="name" class="border rounded p-2 w-full" value="{{ old('name', $user->name) }}">
                        </div>
                        <div class="mb-4">
                            <label class="block text-sm mb-1">Email</label>
                            <input type="email" name="email" class="border rounded p-2 w-full" value="{{ old('email', $user->email) }}">
                        </div>
                        <div class="mb-4">
                            <label class="block text-sm mb-1">Password (opsional)</label>
                            <input type="password" name="password" class="border rounded p-2 w-full" value="">
                        </div>
                        <div class="mb-4">
                            ...
                            <select name="role" class="border rounded p-2">
                                ...
                                <option value="hr" @selected(old('role', $user->role)==='hr')>SDM / HRD</option>
                            </select>
                        </div>
                        <div class="mb-4">
                            <label class="inline-flex items-center">
                                <input type="checkbox" name="is_kepala_kepegawaian" value="1" @checked(old('is_kepala_kepegawaian', $user->is_kepala_kepegawaian))>
                                <span class="ms-2">Kepala Kepegawaian</span>
                            </label>
                        </div>
                        <div class="mb-4">
                            <label class="block text-sm mb-1">Status</label>
                            <input type="text" name="status" class="border rounded p-2 w-full" value="{{ old('status', $user->status) }}" placeholder="active/inactive">
                        </div>
                        <div class="mb-4 grid grid-cols-1 sm:grid-cols-2 gap-4">
                            <div>
                                ...
                                <input type="number" name="points" min="0" max="1000"
                                       ...
                                       value="{{ old('points', $user->points ?? 100) }}">
                            </div>
                            <div>
                                <label class="block text-sm mb-1">Skor Kedisiplinan</label>
                                <input type="number" name="discipline_score" min="0" max="100"
                                       class="border rounded p-2 w-full"
                                       value="{{ old('discipline_score', $user->discipline_score ?? 100) }}">
                            </div>
                        </div>
                        <div class="mb-4">
                            <label class="block text-sm mb-1">No. WhatsApp</label>
                            <input type="text" name="whatsapp_phone" class="border rounded p-2 w-full" value="{{ old('whatsapp_phone', $user->whatsapp_phone) }}">
                        </div>
                        <div class="mb-4">
                            <label class="block text-sm mb-1">Foto Profil</label>
                            <div class="flex items-center gap-4 mt-1">
                                @if ($user->profile_photo_path)
                                    <img src="{{ asset('storage/'.$user->profile_photo_path) }}" alt="Profile photo" class="w-12 h-12 rounded-full object-cover border border-gray-300">
                                @else
                        ...
                        @php
                            $signatureExists = $user->signature_path && \Illuminate\Support\Facades\Storage::disk('public')->exists($user->signature_path);
                        @endphp
                        <div class="mb-4">
                            ...
                            @if ($signatureExists)
                                <div class="mt-2">
                                    <div class="h-16 w-40 bg-white border rounded bg-no-repeat bg-contain bg-left pointer-events-none select-none"
                                         style="background-image: url('{{ asset('storage/'.$user->signature_path) }}');"
